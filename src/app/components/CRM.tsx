...
export function CRM({ onNavigate }: CRMProps) {
  ...
  // Fetch clients from Supabase KV Store
  const fetchClients = async () => {
    try {
      const response = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-662c70dc/clients`, {
        headers: {
      ...
      if (response.ok) {
        ...
        // If empty, fall back to some default visual mock data for the demo
                ...
                <div className="flex justify-between items-center mb-4">
                  <div>
                    ...
                    <div className="text-[#10B981]">
                      {typeof client.revenue === 'number' ? `$${client.revenue.toLocaleString()} CAD` : client.revenue || "$0 CAD"}
                    </div>
                  </div>
                  {client.nextMeeting && (
