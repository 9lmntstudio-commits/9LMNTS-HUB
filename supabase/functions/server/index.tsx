...
app.get("/make-server-662c70dc/auth/verify", async (c) => {
  try {
    ...
    if (!authHeader) {
      console.error(
        "Verification error: No Authorization header",
      );
    ...
    if (!accessToken) {
      console.error(
        "Verification error: No access token in header",
      );
      return c.json({ error: "No access token provided" }, 401);
    }

    console.log(
      "Attempting to verify token with admin client...",
    );
...
app.get("/make-server-662c70dc/admin/users", async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    console.log(
      "Admin users request - Auth header:",
      authHeader ? "Present" : "Missing",
    );
...
// Public Inquiry Endpoint - Save project inquiry (no auth required)
app.post("/make-server-662c70dc/inquiries", async (c) => {
  try {
    ...
    const inquiryId = `inquiry_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
    const inquiry = {
      ...inquiryData,
      id: inquiryId,
      status: "new",
    ...
    console.log(
      `New project inquiry saved: ${inquiryId} from ${inquiryData.email}`,
    );
